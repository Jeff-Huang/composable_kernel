...
// Compose policies left-to-right
...
// Variadic compose over policy templates with optional integral args is complicated; for our use
// we offer a simple variadic template that takes concrete 'apply-able' policy types.

template <typename S, typename... Policies>
struct Compose;
...
template <typename S, typename P0, typename... Rest>
struct Compose<S, P0, Rest...>
{
    using S1   = typename P0::template apply<S>;
    using type = typename Compose<S1, Rest...>::type;
};

// Convenience alias to get final Attribute from Impl and policies

template <typename Impl, WGAttrNumAccessEnum NumAccess, typename... Policies>
using ComposePolicies =
    ComposedAttribute<typename Compose<BaseState<Impl, NumAccess>, Policies...>::type>;
...
struct MakeWarpGemm
{
    using Type = typename MakeWarpGemmHelper<TransposeC,
                                             SwizzleA,
                                             UseStructuredSparsity,
                                             AType,
                                             BType,
